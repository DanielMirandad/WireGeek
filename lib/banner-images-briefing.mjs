...
function contextTerms(banner) {
  /*
   * O angulo editorial do slide vem primeiro.
   *
   * Assim:
   * banner 1 pode ancorar Microsoft/Xbox,
   * banner 2 pode ancorar Activision/Halo,
   * mesmo pertencendo a mesma noticia.
   */
...
function corporateCandidateBlocked(
  candidate
) {
  const imageUrlText =
    normalizeText(
  ...
  /*
   * Para um slide corporativo,
   * URLs explicitamente ligadas a personagem,
   * gameplay ou fan material nao servem.
   *
   * Halo no TITULO da pagina nao basta para rejeitar,
   * porque a materia corporativa pode mencionar Halo.
   * Halo na URL DA IMAGEM, porem, indica fortemente
   * que o asset visual pertence a franquia.
   */
  const blockedUrl =
    /\b(halo|master chief|masterchief|gameplay|screenshot|wallpaper|cosplay|fanart)\b/
      .test(imageUrlText);
...
function buildSearchQueries(
  ...
  if (corporateGameAngle) {
     ...
     * - banner.visual_subject
     *
     * Esses campos carregam o tema geral
     * da materia e poderiam reintroduzir Halo.
     */
    queries = [
      [
        angleTitle,
      ...
      [
        angleTitle,
        "official newsroom corporate press photo Xbox Microsoft studios",
        exclusions,
      ]
        .filter(Boolean)
        .join(" "),
      ...
      [
        angleHighlight,
        "Xbox Microsoft studio office corporate official press photo",
        exclusions,
      ]
        .filter(Boolean)
        .join(" "),
