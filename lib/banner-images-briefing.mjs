...
function buildSearchQueries(
  ...
  const qualifier =
    category === "anime"
      ...
      : category === "games"
        ? "official gameplay screenshot press image"
        : category === "cinema"
          ? "official movie still press image"
          : "official press image promotional still";

  /*
   * Cada slide passa a pesquisar primeiro
   * o proprio angulo editorial.
   *
   * banner_title e highlight ficam antes
   * da query geral da materia.
   */
  const angleTitle =
    String(
      banner.banner_title || ""
