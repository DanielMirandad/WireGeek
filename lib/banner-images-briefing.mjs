  ...
  for (const value of urls) {
    try {
      ...
      const officialMediaDomains = [
        ...
        "news.xbox.com",
        "xbox.com",
        "press.activision.com",
        "ubisoft.com",
        "nintendo.com",
        "ea.com",
        "newsroom.hasbro.com",
        "press.square-enix-games.com",
        "bandainamcoent.com",
        "capcom-games.com",
      ];

      if (
        officialMediaDomains.some(domain =>
          host === domain ||
          host.endsWith("." + domain)
        )
      ) {
        authority = Math.max(authority, 950);
      }
...
function isGettyImageUrl(...values) {
  return values.some(value => {
    try {
      const host = new URL(String(value || "")).hostname.toLowerCase();
      return /(^|\.)gettyimages\.(?:com|ca|co\.uk)$/.test(host);
    } catch {
      return false;
    }
  });
...
function isOfficialPressKitCandidate(candidate) {
  if (!isOfficialPressKitCandidate(candidate)) {
    return false;
  }
  ...
  return [candidate?.source_url, candidate?.url, candidate?.image_url]
    .filter(Boolean)
    .some(value => {
      try {
        const host = new URL(String(value)).hostname.toLowerCase();
...
function hasGettyEditorialLicense(license) {
  const allowedUses = Array.isArray(license?.allowed_uses)
    ? license.allowed_uses.map(value => String(value).toLowerCase())
    : [];

  return (
    license?.status === "licensed" &&
    license?.editorial === true &&
    Boolean(String(license?.license_id || "").trim()) &&
    allowedUses.includes("website") &&
    allowedUses.includes("social")
  );
}
...
async function tryImageUrl({
  ...
  if (
    isGettyImageUrl(normalized) &&
    !hasGettyEditorialLicense(license)
  ) {
    failureReasons.push(
      "Getty Editorial: licença válida para website e redes sociais não confirmada"
    );
    return null;
  }
...
function buildSearchQueries(
  ...
  if (corporateGameAngle) {
    ...
    const officialSearchSubject =
      personCastingProfile
        ? mergeSearchParts(personTargetSeed, productionEntitySeed)
        : primarySubject;

    if (officialSearchSubject) {
      queries.unshift(
        mergeSearchParts(
          officialSearchSubject,
          "official press kit media library high resolution"
        )
      );
    }
...
async function resolveOneBanner({
  ...
  /*
   * PRIORIDADE 2: imagem original extraída da própria matéria.
   * Metadados og:image / JSON-LD e variantes srcset são testados
   * antes de qualquer busca externa.
   */
  const orderedSourceCandidates = [
    ...sourceCandidates.filter(candidate => candidate?.priority === "primary"),
    ...sourceCandidates.filter(candidate => candidate?.priority !== "primary"),
  ];

  for (const candidate of orderedSourceCandidates) {
    const selected = await tryImageUrl({
      url: candidate?.url || candidate,
      banner,
      label: "imagem original da notícia",
      attemptedUrls,
      blockedUrls,
      failureReasons,
    });
  ...
  /*
   * PRIORIDADE 3: press kits oficiais já verificados pelo coletor.
   * O coletor deve marcar source_type como official_presskit.
   */
  for (const candidate of banner.official_image_candidates || []) {
    if (candidate?.source_type !== "official_presskit") {
      failureReasons.push("press kit: origem oficial não verificada");
      continue;
    }

    const selected = await tryImageUrl({
      url: candidate?.url || candidate?.image_url,
      banner,
      label: "press kit oficial",
      attemptedUrls,
      blockedUrls,
      failureReasons,
    });
  ...
  /*
   * PRIORIDADE 4: Getty Editorial apenas com licença registrada
   * para uso editorial no site e nas redes sociais.
   */
  for (const candidate of banner.licensed_editorial_candidates || []) {
    const license = candidate?.license || null;
    if (
      !isGettyImageUrl(candidate?.url, candidate?.source_url) ||
      !hasGettyEditorialLicense(license)
    ) {
      failureReasons.push(
        "Getty Editorial: licença válida para website e redes sociais não confirmada"
    ...
    const selected = await tryImageUrl({
      url: candidate?.url || candidate?.image_url,
      banner,
      label: "Getty Editorial licenciado",
      license,
      attemptedUrls,
      blockedUrls,
      failureReasons,
  ...
  /*
   * --------------------------------------------------------
   * FALLBACK DE URL EXPLÍCITA
   * imagem indicada pelo Briefing, após fontes prioritárias
   * --------------------------------------------------------
   */
  ...
  /*
   * Candidatos já anexados ao briefing ficam depois das fontes
   * prioritárias, mas antes da descoberta externa.
   */
  for (const candidate of banner.image_candidates || []) {
    const item = typeof candidate === "string" ? { url: candidate } : candidate;
    const selected = await tryImageUrl({
      url: item?.url || item?.image_url,
      banner,
      label: "candidato do briefing",
  ...
  /*
   * PRIORIDADE 5: SerpAPI/Brave descobrem candidatos.
   * O resolver baixa a URL original, mede o arquivo e aplica
   * validação semântica/visual; miniaturas nunca são aceitas.
   */
  const externalCandidates = await searchExternalImages({ request, banner });

  for (const candidate of externalCandidates) {
    const selected = await tryImageUrl({
      url: candidate?.url,
      banner,
      label: "candidato descoberto por " + (candidate?.provider || "SerpAPI/Brave") + ": " + (candidate?.title || "resultado"),
      imageProfile: candidate?.search_profile || "default",
