...
function fontPath(env, filename) {
    const candidates = [process.env[env], fileURLToPath(new URL('../public/fonts/' + filename, import.meta.url))];
    if (process.platform === 'win32') candidates.push(join(process.env.WINDIR || 'C:/Windows', 'Fonts', filename));
    const found = candidates.find(value => value && existsSync(value));
    if (!found) throw new Error('Fonte de layout ausente: ' + filename + '. Configure ' + env + '.');
    return found;
}
function makeTextEngine() {
    const fonts = { title: fontPath('BANNER_TITLE_FONT', 'impact.ttf'), body: fontPath('BANNER_BODY_FONT', 'arial.ttf') };
    const cache = new Map();
    return async (text, size, role = 'title', color = '#FFFFFF', spacing = 0) => {
        ...
        if (!cache.has(key)) cache.set(key, sharp({
            text: {
                ...
                font: (role === 'title' ? 'Impact ' : role === 'accent' ? 'Arial Black ' : 'Arial ') + size,
...
export async function renderBanner({ imageBuffer, bannerTitle, shortTitle, headline, category, color = DEFAULT_COLOR }) {
    ...
    let categorySize = 22;
    while (categorySize > 12 && (await raster(clean(category), categorySize, 'body')).info.width > 285) categorySize--;
    await add(clean(category), categorySize, 22, 'body', '#FFFFFF', 0, 30, 285);
    ...
    await add('BAGA\u00C7A', 52, 1314, 'title');
    await add('STUDIOS', 22, 1371, 'title', '#FFFFFF', 8);
