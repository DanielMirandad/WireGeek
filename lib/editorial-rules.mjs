...
export const EDITORIAL_CATEGORIES = Object.freeze(["games", "geek", "cinema", "anime"]);
export const ARTICLE_MIN_CHARS = 500;
export const ARTICLE_MAX_CHARS = 2000;
export const ARTICLE_PARAGRAPH_COUNT = 3;
export const ARTICLE_MIN_PARAGRAPH_CHARS = 80;
...
export const SOCIAL_CANONICAL_CONTRACT = Object.freeze({
  categories: EDITORIAL_CATEGORIES,
  requiredFields: CANONICAL_REQUIRED_FIELDS,
  article: Object.freeze({
    minChars: ARTICLE_MIN_CHARS,
    maxChars: ARTICLE_MAX_CHARS,
    paragraphs: ARTICLE_PARAGRAPH_COUNT,
    minParagraphChars: ARTICLE_MIN_PARAGRAPH_CHARS,
  }),
  highlights: Object.freeze({ count: HIGHLIGHT_COUNT, minWords: MIN_HIGHLIGHT_WORDS, maxWords: MAX_HIGHLIGHT_WORDS }),
  hashtags: Object.freeze({ count: HASHTAG_COUNT, lowercase: true }),
  sources: Object.freeze({ min: SOURCE_MIN_COUNT, max: SOURCE_MAX_COUNT }),
});

export const HIGHLIGHTS_SCHEMA = {
  type: "array", minItems: HIGHLIGHT_COUNT, maxItems: HIGHLIGHT_COUNT,
  description: `Dois ganchos de curiosidade distintos, com ${MIN_HIGHLIGHT_WORDS} a ${MAX_HIGHLIGHT_WORDS} palavras cada e resposta na matéria.`,
  items: { type: "string" },
};
...
export function validateHighlights(highlights, title = "") {
  ...
  highlights.forEach((highlight, index) => {
    ...
    if (words < MIN_HIGHLIGHT_WORDS || words > MAX_HIGHLIGHT_WORDS) errors.push(`${label} precisa de ${MIN_HIGHLIGHT_WORDS} a ${MAX_HIGHLIGHT_WORDS} palavras (recebido: ${words})`);
...
export function validateCanonicalShape(item) {
  const errors = [];
  if (!item || typeof item !== "object") return ["notícia inválida"];
  if (!EDITORIAL_CATEGORIES.includes(item.categoria)) {
    errors.push(`categoria deve ser uma de: ${EDITORIAL_CATEGORIES.join(", ")}`);
  }
  for (const field of ["titulo", "titulo_curto", "manchete_curta", "materia"]) {
    if (!String(item[field] ?? "").trim()) errors.push(`${field} é obrigatório`);
  }
  const articleLength = String(item.materia ?? "").trim().length;
  if (articleLength < ARTICLE_MIN_CHARS || articleLength > ARTICLE_MAX_CHARS) {
    errors.push(`materia deve ter entre ${ARTICLE_MIN_CHARS} e ${ARTICLE_MAX_CHARS} caracteres`);
  }

  const articleParagraphs =
    cleanEditorialText(item.materia)
      .split(/\n\s*\n/)
      .map((paragraph) =>
        paragraph
          .replace(/\s*\n\s*/g, " ")
  ...
  if (
    ...
    errors.push(
      `materia deve conter exatamente ${ARTICLE_PARAGRAPH_COUNT} paragrafos`
    );
  } else if (
    articleParagraphs.some(
      (paragraph) =>
        paragraph.length <
        ARTICLE_MIN_PARAGRAPH_CHARS
    )
  ) {
    errors.push(
      `cada paragrafo da materia deve ter pelo menos ${ARTICLE_MIN_PARAGRAPH_CHARS} caracteres`
    );
  }
  errors.push(...validateHighlights(item.highlights, item.titulo));
  if (!Array.isArray(item.hashtags) || item.hashtags.length !== HASHTAG_COUNT) {
    errors.push(`hashtags devem conter exatamente ${HASHTAG_COUNT} itens`);
  } else if (item.hashtags.some(tag => typeof tag !== "string" || tag !== tag.toLowerCase())) {
    errors.push("hashtags devem estar em minúsculas");
  }
  if (!Array.isArray(item.fontes) || item.fontes.length < SOURCE_MIN_COUNT || item.fontes.length > SOURCE_MAX_COUNT) {
    errors.push(`fontes devem conter entre ${SOURCE_MIN_COUNT} e ${SOURCE_MAX_COUNT} itens`);
  }
  return errors;
}
...
export function validateEditorialItem(item) {
  ...
  for (const field of ["titulo", "materia", "titulo_curto", "manchete_curta"]) {
...
MODELO SOCIAL CANÔNICO:
A categoria deve ser exatamente ${EDITORIAL_CATEGORIES.join(", ")}.
Os campos obrigatórios são titulo, titulo_curto, manchete_curta, materia, highlights, hashtags e fontes.
Escreva materia entre ${ARTICLE_MIN_CHARS} e ${ARTICLE_MAX_CHARS} caracteres, exatamente ${HIGHLIGHT_COUNT} highlights de ${MIN_HIGHLIGHT_WORDS} a ${MAX_HIGHLIGHT_WORDS} palavras,
exatamente ${HASHTAG_COUNT} hashtags em minúsculas e entre ${SOURCE_MIN_COUNT} e ${SOURCE_MAX_COUNT} fontes. Não omita campos nem misture o formato do site Bagaça.

HIGHLIGHTS: exatamente ${HIGHLIGHT_COUNT} strings, cada uma com ${MIN_HIGHLIGHT_WORDS} a ${MAX_HIGHLIGHT_WORDS} palavras.
