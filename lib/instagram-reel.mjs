import { normalizeInstagramProfiles, selectInstagramProfiles } from "./instagram-profiles.mjs";
import {
  mkdtemp,
  readFile,
  rm,
...
export function buildGroupInstagramReelCaption({ article, hashtags, group }) {
  const legacy = buildInstagramReelCaption({ article, hashtags });
  const hasContainer = group.some((row) => row.instagram_parent_container_id);
  const hashes = group.map((row) => String(row.instagram_caption_sha256 || "").toLowerCase());
  // Preservar byte a byte a legenda de containers anteriores a esta funcionalidade.
  if (hasContainer && hashes.every((hash) => hash === legacy.caption_sha256)) return legacy;
  const tagged = buildInstagramReelCaption({
    article, hashtags, profileUsernames: selectInstagramProfiles(article),
  });
  if (hasContainer && !hashes.every((hash) => hash === tagged.caption_sha256)) {
    throw new Error("Legenda ou perfis divergentes do container persistido. Auditoria manual obrigatória.");
  }
  return tagged;
}
