...
function canonicalCandidateImageUrl(value) {
  ...
  try {
     ...
     *
     * O path aponta para o asset de 1280,
     * mas o parametro size for?a a entrega
     * de uma copia reduzida.
     */
    if (
     ...
     * arquivo.jpg
     *
     * Somente dentro de wp-content/uploads.
     */
    if (
     ...
     * Wix:
     *
     * O asset original fica antes do
     * trecho /v1/fill/... ou /v1/fit/...
     */
    if (
      hostname ===
...
function brandLogoImageRejection(candidate, query) {
  ...
  const hasBrandAssetEvidence =
    /\b(logo|logomarca|wordmark|brandmark|branding|brand|identity|identidade|marca|marcacao|mark|credential|credencial)\b/
      .test(searchText);
  ...
  const noiseText = searchText.replace(
    /\b(logo|logomarca|wordmark|brandmark|branding|brand|identity|identidade|marca|marcacao|mark|credential|credencial)\b/g,
    ' '
  );
