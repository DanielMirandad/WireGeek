...
function searchTextRejection(candidate, query, searchProfile) {
  return searchProfile === 'games_corporativo'
    ? corporateImageRejection(candidate, query)
    : candidateMatchesQuery(candidate, query) ? '' : 'relevancia_generica';
}
...
export async function searchImageCandidates(query, { apiKey, fetchImpl = fetch, signal, semanticQuery = query, searchProfile = "default" } = {}) {
  ...
  try {
    ...
    for (const item of raw) {
      ...
      if (textReason) {
        if (searchProfile === 'games_corporativo') console.log('WIRE/GEEK: descarte por texto corporativo', { provider: 'serpapi', titulo: candidate.title, motivo: textReason });
        stats.texto++;
        continue;
      }
  ...
  try {
    ...
    for (const item of raw) {
      ...
      if (
        ...
        if (!(
          Math.min(candidate.width, candidate.height) >= 650 &&
      ...
      const textReason = searchTextRejection(candidate, semanticQuery, searchProfile);
      if (textReason) {
        if (searchProfile === 'games_corporativo') console.log('WIRE/GEEK: descarte por texto corporativo', { provider: 'brave', titulo: candidate.title, motivo: textReason });
        stats.texto++;
        continue;
      }
